...
func (service *DbService) InsertOrUpdateMisinfoReport(p *models.PostModelId) error {
	exists, err := service.MisinfoReportExistsFor(p)
	if err != nil {
		return err
	}

	if exists {
		return service.updateMisinfoReport(p)
	} else {
		return service.insertMisinfoReport(p)
	}
}

func (service *DbService) MisinfoReportExistsFor(p *models.PostModelId) (bool, error) {
	service.dbmut.Lock()
	defer service.dbmut.Unlock()
	...
	var count int64
	row := stmt.QueryRow(p.Id.String())
	if err := row.Scan(&count); err != nil {
		return false, fmt.Errorf("error scanning rows: %v", err)
	}

	return count > 0, nil
}

func (service *DbService) updateMisinfoReport(p *models.PostModelId) error {
	service.dbmut.Lock()
	defer service.dbmut.Unlock()

	stmt, err := service.db.Prepare(`
		update misinfo_report 
		set state=?, confidence=?, date_submitted=?
		where post_id=?
	`)
	if err != nil {
	...
	_, err = stmt.Exec(
		p.MisinfoReport.State,
		p.MisinfoReport.Confidence,
		p.MisinfoReport.SubmittedDateUtc.Format(time.RFC3339),
		p.Id.String(),
	)
...
func (service *DbService) insertMisinfoReport(p *models.PostModelId) error {
	service.dbmut.Lock()
	defer service.dbmut.Unlock()

	stmt, err := service.db.Prepare(`
		insert into misinfo_report(post_id, state, confidence, date_submitted)
		values(?, ?, ?, ?)
	`)
	if err != nil {
		return fmt.Errorf("error preparing sql statement: %v", err)
